...
export function MonthView({ currentDate, bookings, onBookingClick }: MonthViewProps) {
  const days = useMemo(() => {
      ...
      <div className="grid grid-cols-7 flex-1" style={{ gridAutoRows: "minmax(100px, 1fr)" }}>
        {days.map((day, idx) => {
          ...
          return (
            <div
              key={key}
              className={`border-r border-b border-border p-1.5 flex flex-col gap-1 min-h-0 overflow-hidden ${
                !inMonth ? "bg-muted/20" : ""
