  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [siteRes, pgRes, sessRes, galRes, pagesRes] = await Promise.all([
        ...
        supabase.from("sessions").select("id, slug, title, description, tagline, price, duration_minutes, num_photos, location, cover_image_url").eq("photographer_id", user.id).eq("status", "active").neq("hide_from_store" as any, true),
