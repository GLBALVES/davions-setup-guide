 ...
 * public site using all photographer_site configuration AND the published
 * snapshot from site_pages (so the Publish button in the editor actually
 * propagates to the live custom domain).
 */
...
const CustomDomainStore = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Public custom domain reads the published snapshot; ?preview=1 reads the live draft.
  ...
  return (
    <PublicSiteRenderer
      ...
      pageHeaderConfig={(homeHeaderConfig as any) ?? null}
    />
  );
};
