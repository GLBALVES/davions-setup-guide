  ...
  return (
    <div className="relative group rounded-lg overflow-hidden border border-white/10 bg-[#0d1117] my-3">
      <div className="flex items-center justify-between px-4 py-2 bg-white/5 border-b border-white/10">
        ...
        <button onClick={copy} className="flex items-center gap-1.5 text-[10px] text-white/40 hover:text-white/70 transition-colors">
...
function Badge({ label, color }: { label: string; color: "green" | "blue" | "yellow" | "muted" }) {
  ...
  return <span className={cn("inline-block text-[10px] tracking-wider uppercase font-mono px-2 py-0.5 rounded border", styles[color])}>{label}</span>;
}
...
function DocSection({ id, title, icon: Icon, children }: { id: SectionId; title: string; icon: React.ElementType; children: React.ReactNode; }) {
  return (
    <section id={`docs-${id}`} className="scroll-mt-6 mb-12">
      ...
      <table className="w-full text-xs font-mono">
        <thead>
          <tr className="border-b border-white/10 bg-white/5">
            {headers.map((h) => <th key={h} className="text-left px-4 py-2.5 text-[10px] tracking-widest uppercase text-white/40 font-light">{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className={cn("border-b border-white/5 last:border-0", i % 2 === 0 ? "bg-transparent" : "bg-white/[0.02]")}>
              {row.map((cell, j) => <td key={j} className="px-4 py-2.5 text-white/70">{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
...
function Callout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex gap-3 rounded-lg border border-yellow-500/20 bg-yellow-500/5 px-4 py-3 text-xs leading-relaxed mb-4 text-yellow-200/80">
      ...
      <main className="flex-1 overflow-y-auto px-10 py-8">
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-[10px] tracking-[0.3em] uppercase text-white/30 font-light">Documentação de Referência</p>
          </div>
          <h1 className="text-2xl font-light text-white/90">Infraestrutura VPS</h1>
          <p className="text-xs text-white/30 mt-1 font-light">Referência técnica de operação do servidor de produção.</p>
        </div>

        <DocSection id="overview" title="Visão Geral" icon={Server}>
          <div className="grid grid-cols-2 gap-3 mb-5">
            {[{ label: "IP do Servidor", value: "147.93.112.182" }, { label: "Hostname", value: "srv700943" }, { label: "Sistema Operacional", value: "Ubuntu 24 LTS" }, { label: "Gerenciador", value: "Easypanel" }].map(({ label, value }) => (
        ...
        <DocSection id="architecture" title="Arquitetura" icon={Network}>
          <p className="text-xs text-white/50 leading-relaxed mb-4 font-light">Fluxo de uma requisição de domínio customizado:</p>
          ...
          <p className="text-xs text-white/50 leading-relaxed mb-4 font-light">
            O Smart Proxy consulta o Supabase para resolver o destino, injetando o header <code className="text-blue-300 font-mono text-[11px]">X-Forwarded-Host</code>.
          </p>
          <p className="text-[10px] tracking-widest uppercase text-white/30 mb-2 font-light">Lógica de resolução</p>
          <DocTable
        ...
        <DocSection id="smart-proxy" title="Smart Proxy" icon={Cpu}>
          <div className="flex gap-3 mb-4 flex-wrap">
            {[{ label: "Código", value: "/etc/smart-proxy/server.js" }, { label: "Container", value: "/etc/smart-proxy/Dockerfile" }].map(({ label, value }) => (
              <div key={label} className="rounded border border-white/10 bg-white/[0.03] px-3 py-2">
                <p className="text-[9px] text-white/30 uppercase tracking-widest font-light mb-0.5">{label}</p>
                <code className="text-[11px] font-mono text-white/70">{value}</code>
              </div>
            ))}
          </div>
          <p className="text-[10px] tracking-widest uppercase text-white/30 mb-2 font-light">Endpoints</p>
          <DocTable headers={["Método", "Path", "Descrição"]} rows={[["GET", "/api/certs", "Lista certificados SSL do acme.json do Traefik"], ["GET", "/docs", "Página de status e documentação inline"]]} />
  ...
  smart-proxy`} />
        ...
        <DocSection id="traefik" title="Traefik" icon={Network}>
          <Callout>
            ...
            Novos routers devem ser adicionados via script Python ao <code className="font-mono">/etc/easypanel/traefik/config/main.yaml</code>.
          </Callout>
        ...
        <DocSection id="new-app" title="Adicionar Novo App" icon={PlusCircle}>
          <p className="text-xs text-white/50 leading-relaxed mb-4 font-light">Passos para suportar um terceiro app no Smart Proxy:</p>
          <ol className="flex flex-col gap-3 text-xs text-white/60 font-light">
            {[
              ...
              <li key={i} className="flex gap-3">
                <span className="w-5 h-5 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[10px] font-mono text-white/40 shrink-0 mt-0.5">{i + 1}</span>
                <span>{text}</span>
              </li>
            ))}
          </ol>
  ...
  return davionsProxy(req, res);
        ...
        <DocSection id="troubleshooting" title="Troubleshooting" icon={Wrench}>
          <div className="flex flex-col gap-5">
            {[
              { title: "\"Domain Not Configured\" inesperado", items: ["Verificar se o domínio está cadastrado no banco correto.", "Confirmar domain_verified = true na tabela do Gleamhub.", "Aguardar expiração do cache (5 min) ou reiniciar o container."] },
              { title: "Erro SSL 526 (Cloudflare)", items: ["No painel Cloudflare, mudar para DNS Only (nuvem cinza).", "Verificar se o registro A aponta para 147.93.112.182.", "Aguardar propagação DNS (pode levar até 30 min)."] },
            ].map(({ title, items }) => (
              <div key={title} className="rounded-lg border border-white/10 bg-white/[0.02] overflow-hidden">
                <div className="px-4 py-3 border-b border-white/10 bg-white/[0.03]">
                  <p className="text-xs font-mono text-yellow-300/80">⚠ {title}</p>
                </div>
